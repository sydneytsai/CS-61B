...
class Board {
    ...
    void clear() {
        ...
        _numJumps = 0;
        incrPieces(RED, numPieces(RED) * -1);
        incrPieces(BLUE, numPieces(BLUE) * -1);
        incrPieces(RED, 2);
        incrPieces(BLUE, 2);
        _winner = null;
        announce();
    ...
    boolean canMove(PieceColor who) {
        for (int i = 0; i < _board.length; i += 1) {
            if (_board[i] == who) {
                ...
                for (int j = i - 13; j <= i - 9; j++) {
                    if (_board[j] == EMPTY) {
                        return true;
                    }
                }
                for (int j = i + 9; j <= i + 13; j++) {
                    if (_board[j] == EMPTY) {
                        return true;
                    }
                }
    ...
    void makeMove(Move move) {
        ...
        if ((!canMove(RED) && !canMove(BLUE)) || numJumps() > JUMP_LIMIT) {
    ...
    void makeMoveHelper(Move move) {
        ...
        for (int i = move.toIndex() - 1; i < move.toIndex() + 2; i++) {
            if (_board[i] == opponent) {
                addUndo(i);
                set(i, _whoseMove);
                incrPieces(_whoseMove, 1);
        ...
        for (int i = move.toIndex() + 10; i < move.toIndex() + 13; i++) {
            if (_board[i] == opponent) {
                addUndo(i);
                set(i, _whoseMove);
                incrPieces(_whoseMove, 1);
    ...
    void undo() {
        _whoseMove = _whoseMove.opposite();
        if (_undoPieces.isEmpty() || _undoSquares.isEmpty()) {
            return;
        }

        Integer prevSquare = _undoSquares.pop();
        PieceColor prevPiece = _undoPieces.pop();
        Move lastMove = _allMoves.get(allMoves().size() - 1);
        if (lastMove.isPass()) {
            allMoves().remove(allMoves().size() - 1);
            announce();
            return;
        } else {
            while (prevSquare != null && prevPiece != null) {
                _board[prevSquare] = prevPiece;
                prevSquare = _undoSquares.pop();
                prevPiece = _undoPieces.pop();
            }
            if (lastMove.isJump()) {
                _board[lastMove.fromIndex()] = _whoseMove;
                _numJumps -= 1;
            }
    ...
    void setBlock(char c, char r) {
        ...
        if (_board[index(c, r)] == EMPTY) {
            char cm = (char) ('a' + 'g' - c);
            char rm = (char) ('1' + '7' - r);
            boolean first = legalBlock(cm, r);
            boolean second = legalBlock(c, rm);
            boolean third = legalBlock(cm, rm);
            ...
            if (!(first && second && third)) {
                ...
                _board[index(cm, r)] = BLOCKED;
                _board[index(c, rm)] = BLOCKED;
                _board[index(cm, rm)] = BLOCKED;
                if (c != cm) {
                    _totalOpen -= 1;
                }
                if (r != rm) {
