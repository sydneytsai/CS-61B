...
public class CompoundInterest {
    ...
    static double totalSavings(double perYear, int targetYear, double rate) {
        ...
        for (int i = 1; i <= numYears(targetYear); i++){
            double mult = 1.0;
            for (int j = i; j > 0; j--){
                mult = mult * (1.0 + rate/100);
            }
            ans = ans + (perYear * mult);
        }
        return ans;
    }
    ...
    static void printSavingsFV(int targetYear, double returnRate,
        ...
        double nominalSavings = futureValue(totalSavings(perYear, targetYear, returnRate), returnRate, targetYear); // replace 0 with your code
        double realSavings = futureValueReal(totalSavingsReal(perYear, targetYear, returnRate, inflationRate), returnRate, targetYear, perYear);    // replace 0 with your code
